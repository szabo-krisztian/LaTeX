...
\begin{document}
    ...
    Mindezt figyelembe véve legyen
    \[
        f : I_1 \times I_2 \to \R
    \]
    adott "kétváltozós" függvény, és tetszőleges $x \in I_1, \, y \in I_2$ esetén tekintsük az
    \[
        f_x : I_2 \to \R,
    \]
    ill. az
    ...
    függvényt, ahol
    \[
        f_x(z) := f(x, \, z) \quad (z \in I_2),
    \]
    ill.
    \[
        f_y(t) := f(t, \, y) \quad (t \in I_1).
    \]
    Ha az $f$ korlátos, akkor nyilván az $f_x, \, f^y \, \, (x \in I_1, \, y \in I_2)$ függvények is korlátosak. Következésképpen vehetjük azokat az
    \[
        \mathbf{m}_f, \mathbf{M}_f : I_1 \to \R, \text{ valamint } \mathbf{m}^f, \, \mathbf{M}^f : I_2 \to \R
    \]
    függvényeket, amiket az alábbi módon értelmezünk:
    \[
        \mm_f(x) := I_\star(f_x) \quad (x \in I_1), \quad \, \MM_f(x) := I^\star(f_x) \quad (x \in I_1),
    \]
    \[
        \mm^f(y) := I_\star(f_y) \quad (y \in I_2), \quad \, \MM^f(y) := I^\star(f_y) \quad (y \in I_2).
    \]
    A most bevezetett jelölésekkel a szukcesszív integrálás tétele a következőképpen szól:

    \subsection{Fubini-tétel}
    \tikz \node[theorem]
    {
        \textbf{Tétel.} Tegyük fel, hogy $f \in R(I).$ Ekkor
        \[
            \mm_f, \, \MM_f \in R(I_1) \text{ és } \mm^f, \, \MM^f \in R(I_2),
        \]
        \textit{továbbá}
        \[
            \int_{I_1} \mm_f = \int_{I_1} \MM_f =\int_{I_2} \mm^f = \int_{I_2} \MM^f = \int_{I}f.
        \]
    };
    ...
    \section{Felület, felszín. Felületi integrál, fluxus.}
    \subsection{Felszín}
    Tegyük fel, hogy a $\Phi : [0, \, 1]^2 \to \R^3$ függvény folytonosan differenciálható, és a
