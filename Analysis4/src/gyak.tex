...
\begin{document}
	...
	\tikz \node[definition]
	{
		\textbf{Definíció.} Legyen $1 \leq s \in \N, \, A \subset \R^s$. Azt mondjuk, hogy az $A$ halmaz \textit{nullamértékű}, ha tetszőleges $\varepsilon > 0$ számhoz megadható $I_k \subset \R^s \, (k \in \N)$ intervallumoknak egy olyan sorozata, hogy
		\[
	...
	\]
	Ekkor a $\text{graf} \, f \subset \R^{n+1}$ halmaz nullamértékű.\\
	...
	\begin{enumerate}
		\item Ha $\mathcal{F}_1 := \mathcal{R}_{\Psi_1}$, ill. $\mathcal{F}_2 := \mathcal{R}_{\Psi_2}$ nullamértékű halmazban (pl. élekben) csatlakozó felületdarabok, akkor
		\[
	...
	\]
	függvénysorozattal legyen $0 < \delta < 1$ és $A := [-\delta, \, \delta]$, amikor
	\[
		\sum_{n=0}^\infty \sup\{ |h_n(x)| : x \in A\} = \sum_{n=0}^\infty \sup\{|x^n| : x \in [-\delta, \, \delta]\} = \sum_{n=0}^\infty \delta^n < + \infty.
	\]
	Tehát a szóban forgó $\sum(h_n)$ függvénysor a $[-\delta, \, \delta]$ intervallumon egyenletesen konvergens.\\
	...
	\]
	akkor az általuk meghatározott $\sum(a_n \cos(nx) + b_n \sin(nx))$ trigonometrikus sor egyenletesen konvergens.

	\subsubsection{Folytonossági tétel}
	\tikz \node[theorem]
	{
		...
		\[
			f : X \to \K
		\]
		függvény. Ha $a \in X$ és $f_n \in \mathcal{C}\{a\} \quad (n \in \N)$, akkor $f \in \mathcal{C}\{a\}$. 
	};\\
	...
	\tikz \node[theorem]
	{
		...
		\]
		függvények által meghatározott $\sum(g_n)$ függvénysor, ami egyenletesen konvergens, az összegfüggvénye pedig a
		\[
			G : X \to \K
		\]
		függvény. Ha $a \in X$ és $g_n \in \mathcal{C}\{a\} \quad (n \in \N)$, akkor $G \in \mathcal{C}\{a\}$. 
	};
	
	\subsubsection{Integrálhatósági tétel}
	\tikz \node[theorem]
	{
		\textbf{Tétel.} Tekintsük az $[a, \, b] \quad (a, \, b \in \R, \, a < b)$ intervallumon értelmezett $f_n : [a, \, b] \to \R \quad (n \in \N)$ függvényekből álló $(f_n)$ függvénysorozatot, amelyikről tegyük fel, hogy egyenletesen konvergens. Ha $f_n \in R[a, \, b] \quad (n \in \N)$ és $f$ jelöli az $(f_n)$ sorozat határfüggvényét, akkor $f \in R[a, \, b]$, az integrálok $(\int\limits_a^b f_n)$ sorozata konvergens, és
		\[
	...
	Legyen adott $(g_n)$ a fentebb említett sorozat. Ekkor 
	\[
		G_n(x) := \sum_{k=0}^n g_k(x) = g_0(x) + g_1(x) + \cdots +  g_n(x) \quad (x \in [a, \, b]).
	\]
	$G_n \equiv \sum(g_n)$. Legyen
	\[
		G(x) := \lim_{n \to \infty} G_n(x) \quad (x \in [a, \, b]).
	\]
	Adott $x \in [a, \, b]$ mellett
	\[
		\int\limits_a^b G_n(x) \, dx = \int\limits_a^b \left( \sum_{k=0}^n g_k(x) \right) \, dx = \sum_{k=0}^n \int\limits_a^b g_k(x) \, dx = \int\limits_a^b g_0(x) \, dx + \cdots + \int\limits_a^b g_n(x) \, dx.
	\]
	Ekkor
	\[
		\int\limits_a^b G(x) \, dx = \sum_{n=0}^\infty \int\limits_a^b g_n(x) \, dx,
	\]
	azaz
	\[
		\int\limits_a^b \lim_{n \to \infty} G_n(x) \, dx = \lim_{n \to \infty} \sum_{k=0}^n \int\limits_a^b g_k(x) \, dx = \lim_{n \to \infty} \int\limits_a^b G_n(x) \, dx.
	\]
	Részletesebben kiírva:
	\[
		\int\limits_a^b (g_0(x) + g_1(x) + \cdots) \, dx = \int\limits_a^b g_0(x) \, dx + \int\limits_a^b g_1(x) \, dx + \cdots.
	\]
